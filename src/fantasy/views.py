...
def index(request):
    return render(request, 'fantasyPage.html')
...
def addPlayer(request):
    ...
    if player.empty or player_already_on_team==True:
        ...
        print(1)
        cap_space = 40000-team_df['salary'].sum()
        ...
        if len(team_df)!=0:
            curr_cap = 40000-team_df['salary'].sum()
            if curr_cap<=0:
                cap_space = 40000-team_df['salary'].sum()
                player_count = 10-len(team_df)
                done=True
        ...
        if done==False:
            ...
            cap_space = 40000-team_df_['salary'].sum()
...
def submit_team(request):
    ...
    new_team = user_created_teams(mean_age=team_age, mean_scoring=team_scoring, mean_passing=team_passing, mean_rebounding=team_rebounding, mean_rating=team_rtg, mean_efficiency=team_eff, 
    mean_turnOver_grade=team_turnOver,mean_overall=team_overall,total_spendings=total_cap)
    new_team.save()
    return render(request, 'teamReport.html', {'team_':team_, 'cap_space':cap_space,
